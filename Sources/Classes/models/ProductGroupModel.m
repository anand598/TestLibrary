...
- (NSArray*)allImageURLs {
    ...
    for (ProductEntityModel* e in self.entities) {
...
- (NSArray*)entities {
    ...
    for (NSDictionary* entity in [entityInfos objectEnumerator]) {
        ProductEntityModel* model = [[ProductEntityModel alloc] initWithInfo:entity];
        model.parent = self;
...
- (ProductEntityModel*)firstEntity{
    if(self.onPageVariant){
        if (self.entities && [self.entities count]) {
            for(ProductEntityModel* p in self.entities){
                if([p.ID isEqualToString:self.onPageVariant]){
                    return p;
                }
            }
        }
...
- (BOOL)isSale {
    CGFloat pf = self.priceFloat;
    CGFloat sf = self.priceSaleFloat;
    return (sf != 0.0 && sf < pf);
}
...
- (NSArray*)altImageURLs {
    ...
    for (NSString* get in getters) {
        ...
        if (alts && [alts respondsToSelector:@selector(indexOfObject:)]) {
            NSArray* urls = @[];
            BOOL shouldSearchDicts = NO;
            for (NSString* alt in alts) {
                if([alt isKindOfClass:[NSString class]]){
                    urls = [urls arrayByAddingObject:[NSURL URLWithString:alt]];
                } else {
                    shouldSearchDicts = YES;
                }
            }
            if(shouldSearchDicts == YES){
                for (NSDictionary* alt in alts) {
                    NSString* str = [alt objectForKey:@"Url"];
                        urls = [urls arrayByAddingObject:[NSURL URLWithString:str]];
                }
            }
            return urls;
        }
...
//returns an array of SizeModels
- (NSArray*)sizes{
    NSArray* sizes = @[];
    
    for (ProductEntityModel* entity in [self entities]) {
        SizeModel* size = [[SizeModel alloc] init];
        size.name = [entity.info stringByKey:@"Size"];
        if (!size.name) {
...
//        }
        ...
        sizes = [sizes arrayByAddingObject:size];
    }
...
- (NSString*)shippingInfo{
    if(self.firstEntity){
        return self.firstEntity.shippingInfo;
    }
