...
- (void)updateLayout {
    ...
    //THIS FIXES WEIRD FIRST TIME CATALOG VIEW CONTROLLER BUG FOR ERTICAL LAYOUT CELLS
    dispatch_async(dispatch_get_main_queue(), ^{
        CGRect fullScreen = [[UIScreen mainScreen] bounds];
        self.indicatorView.frame = [self frameForIndicatorView];
        self.pageView.frame = self.frame;//pageViewFrame;
        self.pageViewMask.frame = [self frameForPageViewMask];
        self.productsView.frame = [self frameForProductsView];
        self.scrollView.frame = fullScreen;
        self.zoomBackgroundView.frame = fullScreen;
        self.scrollView.contentSize = fullScreen.size;
        [self setNeedsLayout];
...
- (void)setPageModel:(PageModel *)pageModel {
    ...
    if (!pageModel) {
        NSLog(@"THERE IS NO PAGE MODEL FOR THIS CELL");
        return;
    }
...
- (void)pageView:(id)pageView didSelectElement:(ElementModel *)element {
    switch (element.type) {
        ...
        case ElementModelType_Product:
        case ElementModelType_Variant:
        case ElementModelType_Link: {
            if(self.elementSelectionDelegate){
                if([self.elementSelectionDelegate respondsToSelector:@selector(pageView:didSelectElement:)]){
                    [self.elementSelectionDelegate pageView:pageView didSelectElement:element];
                }
            }
            break;
        }
