...
@implementation TrackingConclusion
...
- (void)trackStartVideo:(VideoModel*)v {
    ...
    TrackingEventModel* e = [[TrackingEventModel alloc] initWithEvent:@"video-start" path:path andData:nil];
    [self addEvent:e];
}
...
- (void)trackSearchPhrase:(NSString *)s {
    TrackingEventModel* e = [TrackingEventModel trackEvent:@"search"
                             inCatalog:self.catalogModel
                              withData:@{@"searchString" : s}];
    e.extras = @{ @"phrase" : s};
    [self addEvent:e];
}
...
- (void)trackViewGuide {
    TrackingEventModel* e = [TrackingEventModel trackEvent:@"view"
                                              withGuideKey:[TrackProxy guideKey]
                                                   andData:nil];
    [self addEvent:e];
}
...
- (void)trackNavTap:(NSString *)item {
    ...
    TrackingEventModel* e = [TrackingEventModel trackEvent:@"nav-click"
                                              withGuideKey:[TrackProxy guideKey]
                                                   andData:@{@"item":_item}];
    [self addEvent:e];
}
...
- (void)trackViewPage {
    if (self.catalogModel && self.pageModels) {
        ...
        TrackingEventModel* e = nil;
        NSArray* videoModels = leftPage.videoModels;
        if ([self.pageModels count] == 2) {
            e = [TrackingEventModel viewSpreadWithLeftPage:leftPage ofCatalog:self.catalogModel];
            PageModel* rightPage = self.pageModels[1];
            videoModels = [videoModels arrayByAddingObjectsFromArray:rightPage.videoModels];
        } else {
            e = [TrackingEventModel viewPage:leftPage ofCatalog:self.catalogModel];
        }
    ...
    if (self.productModel && self.catalogModel && self.pageModels) {
        for (PageModel* page in self.pageModels) {
            for (ElementModel* element in page.elementModels) {
                if ([element.productID isEqualToString:self.productModel.ID]) {
                    self.elementModel = element;
                    [self addEvent:[TrackingEventModel  tapProduct:self.productModel
                                                        ofElement:self.elementModel
                                                           onPage:page
                                                        inCatalog:self.catalogModel]];
                    return;
...
static TrackProxy* __defaultTrackProxy = nil;

+ (TrackProxy*)defaultProxy {
    if (!__defaultTrackProxy) {
        __defaultTrackProxy = [[TrackProxy alloc] initWithInterval:INTERVAL];
        __defaultTrackProxy.saveResponses = NO;
        __defaultTrackProxy.conclusions = @[];
    }
    return __defaultTrackProxy;
}
...
- (void)reconcileResponse:(TrackingResponseModel*)response withCache:(NSArray*)cache {
    ...
    for (TrackingEventModel* event in cache) {
        TrackingConclusion* c = [[TrackingConclusion alloc] init];
        c.event = event;
        c.response = response;
